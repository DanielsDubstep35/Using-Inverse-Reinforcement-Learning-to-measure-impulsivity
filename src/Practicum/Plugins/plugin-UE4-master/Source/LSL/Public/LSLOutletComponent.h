...
public:
	...
	/**
	* Whether to automatically initialize the outlet on BeginPlay or to do it manually
	*/
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = LSL, meta = (ExposeOnSpawn = "true"))
    bool bAutoInitOutlet = true;
    ...
    virtual void InitOutlet();

    UFUNCTION(BlueprintCallable, Category = LSL)
    virtual void PushSampleFloat(TArray<float> data);

    //UFUNCTION(BlueprintCallable, Category = LSL)
    //void PushSampleDouble(TArray<double> data);

    UFUNCTION(BlueprintCallable, Category = LSL)
    virtual void PushSampleLong(TArray<int32> data);
    ...
    UFUNCTION(BlueprintCallable, Category = LSL)
    virtual void PushSampleString(TArray<FString> data);
...
protected:
	...
	TSharedPtr<lsl::stream_outlet> my_outlet = nullptr;

};

template<typename T>
inline void ULSLOutletComponent::PushSample(TArray<T> data) {
	if (!my_outlet.IsValid()) {
	...
	if (data.Num() != Channels.Num()) {
		UE_LOG(LogLSL, Warning, TEXT("Trying to push sample with invalid size to %s, skipping"), *(GetFName().ToString()));
		return;
	}
